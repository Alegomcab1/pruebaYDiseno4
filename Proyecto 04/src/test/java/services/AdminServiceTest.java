...
public class AdminServiceTest extends AbstractTest {
	...
	public void testSaveAdmin() {
		super.authenticate("admin");
		//--------------ESTOS DOS CONSTRUCTORES------------
		Admin admin = new Admin();
		Admin savedAdmin = new Admin();
		//-------------------------------------------------
		admin = this.adminService.createAdmin("name", "middleName", "surname", "https://www.fakeUrl.com", "fakeEmail@fakeDomain.com", "phoneNumber", "address", "userName", "password");
	...
	public void testUpdateWarranty() {
		...
		//Creamos una warranty (ya se ha comprobado que no da error en la prueba anterior
		Warranty warranty = this.adminService.createWarranty("warrantyTest");
		Warranty saved = this.adminService.saveWarranty(warranty);
	...
	public void testBroadcastMessage() {
		...
		for (Actor a : allActors) {
			...
			for (Message m : receivedMessages)
				if (m.getBody().equals(saved.getBody()) && m.getSubject().equals(saved.getSubject()) && m.getSender().equals(saved.getSender()) && m.getReceivers().containsAll(saved.getReceivers()))
					count = count + 1;
